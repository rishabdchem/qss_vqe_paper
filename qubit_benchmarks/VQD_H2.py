...
g = {0.35: [7.01273E-01, -7.47416E-01, 1.62573E-01, -7.47416E-01, 1.31036E-02],
...
def SSVQE(params, ansatz, hamiltonian, get_energy=False):
    estimator = Estimator(options={"shots": 2**10})
    loss = 0
...
all_stdevs = []
all_exact_energies = []
my_params = 2*np.pi*np.random.rand(16) # Randomly Initialize Parameters
...
for R in hhdis_ref:
    ...
    optimizer = L_BFGS_B(maxiter=max_iter)#SLSQP()

    k = 3 # Number of eigenvalues to determine
    betas = [10, 10, 10] # Deflation Hyperparameter for each found state
